...
def e2e_stubbed_openai(monkeypatch) -> Iterator[None]:
    ...
    mapping = {
        "what is our total revenue this month?": (
            "SELECT SUM(total_amount) AS revenue FROM orders " "WHERE order_date >= DATE('now', '-30 day');"
        ),
        ...
        "what are the top 5 selling products?": (
            "SELECT product_name, COUNT(id) AS sales FROM orders " "GROUP BY product_name ORDER BY sales DESC LIMIT 5;"
        ),
        ...
        "how many new customers joined last week?": (
            "SELECT COUNT(*) AS new_customers " "FROM customers WHERE created_at >= DATE('now', '-7 day');"
        ),
    }
